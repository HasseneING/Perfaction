...
visite::visite(int id,QString matricule,QString compagnie,int code,QDateTime dateDeb,
               QDateTime dateExp,double prix,int courante)
{
    this->id=id;
...
QSqlQueryModel* visite::afficher()
{
    QSqlQueryModel *model = new QSqlQueryModel;
      model->setQuery("SELECT v.ID,v.MATRICULE,vt.MARQUE,vt.MODELE,v.COMPAGNIE,v.CODE,"
                      "v.DATE_DEB,v.DATE_EXP,v.PRIX,v.courante FROM visite v INNER JOIN voitures vt "
...
bool visite::ajouter()
{
    QSqlQuery query;
      ...
      query.prepare("INSERT INTO visite (ID, MATRICULE,COMPAGNIE,CODE, DATE_DEB,DATE_EXP ,PRIX ,COURANTE) "
                    "VALUES (:ID,:MATRICULE,:COMPAGNIE,:CODE,:DATE_DEB,:DATE_EXP ,:PRIX ,:COURANTE)");
